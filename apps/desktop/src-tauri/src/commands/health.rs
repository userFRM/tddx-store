...
pub struct HealthSnapshot {
    /// The account-wide in-flight budget the pool runs at.
    pub pool_size: u32,
...
pub async fn health(
    ...
    // Pool sizing is derived from the live ThetaData subscription, not
    // from a user setting. Pre-connect there is no subscription to read,
    // so `UserTiers::default()` reports the most restrictive budget
    // rather than a number the server has not agreed to.
    let tiers = state
        .client
        .read()
        .await
        ...
        .unwrap_or_default();
    Ok(HealthSnapshot {
        pool_size: tiers.in_flight_budget() as u32,
